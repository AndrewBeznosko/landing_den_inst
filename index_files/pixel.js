window.AMOPIXEL_IDENTIFIER || (function () {
    ...
    stringify: function (r) {
      if (r instanceof Object) {
        ...
        if (r.constructor === Array) {
          for (var n = 0; n < r.length; t += this.stringify(r[n]) + ",", n++);
          return "[" + t.substr(0, t.length - 1) + "]"
        }
        if (r.toString !== Object.prototype.toString) return '"' + r.toString().replace(/"/g, "\\$&") + '"';
        for (var i in r) t += '"' + i.replace(/"/g, "\\$&") + '":' + this.stringify(r[i]) + ",";
        return "{" + t.substr(0, t.length - 1) + "}"
      }
      return "string" == typeof r ? '"' + r.replace(/"/g, "\\$&") + '"' : String(r)
    }
  });
    ...
    function n(t) {
      function o(n, r, i) {
        ...
        for (var p = document.cookie ? document.cookie.split("; ") : [], d = /(%[0-9A-Z]{2})+/g, u = 0; u < p.length; u++) {
          ...
          try {
            if (m = t.read ? t.read(m, l) : t(m, l) || m.replace(d, decodeURIComponent), this.json)
              try {
                m = JSON.parse(m)
              } catch (a) {}
            if (n === l) {
   ...
   * Константы модуля
   */
  var PIXEL_IDENTIFIER_JS_ID = 'ss_pixel_identifier_js';
  ...
  var Identifier = function () {
    ...
    var pa = identifier_script.src.split("?").pop().split("&")

    var pixel_params = {};
    for (var j = 0; j < pa.length; j++) {
      var kv = pa[j].split("=");
      pixel_params[kv[0]] = kv[1];
    }

    var ssuid = Fn.getParameterByName('ssuid')
    var ss_visitor_uid = window.Cookies.get('ss_visitor_uid') || null

    var data = {}
    if (ssuid) data.ssuid = ssuid
    if (ss_visitor_uid) data.ss_visitor_id = ss_visitor_uid
    data = Object.assign(data, pixel_params)
    ...
    if (typeof window['SS_PIXEL_IDENTIFIER_PARAMS'] === 'object' && window['SS_PIXEL_IDENTIFIER_PARAMS'] !== null) {
      if (typeof window['SS_PIXEL_IDENTIFIER_PARAMS']['onload'] === 'function') {
        Data.callbacks.onload = window['SS_PIXEL_IDENTIFIER_PARAMS']['onload'];
      }
    }
    console.log('base host', Data.base_host )

    reqwest({
      url: Data.base_host + '/pixel_logic',
      method: 'post',
      data: data,
      success: function (res) {
        console.log('response', res)
        console.log('ss_visitor_uid', res['ss_visitor_uid'], 'ss_customer_uid',  res['ss_customer_uid'])

        if (!(res && res['ss_visitor_uid'] && res['ss_customer_uid'])) {
          console.log('no sid')
          return;
        }

        console.log('add uid toData')
        Data.visitor_uid = res['ss_customer_uid'];
        console.log('add to cookies')
        window.Cookies.set('ss_visitor_uid', res['ss_visitor_uid'])

        Data.callbacks.onload(window.SS_PIXEL_IDENTIFIER);
  ...
  window.SS_PIXEL_IDENTIFIER = new Identifier();
